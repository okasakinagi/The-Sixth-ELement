...
### 2.14 IDVECTOR（对象向量存储）

**用途**：保存与 `user` 或 `survey` 相关联的向量（embedding），供相似度计算、个性化推荐等使用。当前设计将向量以 `float32` 二进制（`BinaryField`）存储在关系库中，适用于向量维度较小且经常整体加载的场景。

**关键字段**

* `id`：主键
* `ref_type`：引用类型，`user` 或 `survey`
* `ref_id`：对应对象 ID（字符串）
* `vector`：二进制存储的 `float32` 数组（按顺序存储，读取时反序列化为浮点数组）
* `created_at`：写入时间（用于判断用户向量是否为当天生成）

**索引**

* 联合索引 `(ref_type, ref_id)`，用于快速定位对象向量
* 索引 `(ref_type, created_at)`，用于按时间查询或筛选

**业务注意**

* 对于 `user`，系统在读取时会判断 `created_at` 是否为当天以决定是否复用向量；对于 `survey`，只要存在向量即可复用。
* 若后续有大规模检索需求，建议将向量同步到专用向量库并在关系库保留元数据或外部向量 ID。

---

### 2.15 SURVEY_USER_SIMILARITY（问卷-用户相似度缓存）

**用途**：缓存 `survey` 与 `user` 之间的相似度（cosine），按天生成并覆盖，用于减少重复计算并加速推荐。
...
* `user_id`：FK → `USER`
* `cosine`：浮点数，余弦相似度
* `created_at`：生成时间（用于判断是否为今日缓存）

**约束与索引**

* `UNIQUE(survey_id, user_id)`：每对问卷-用户仅保留一条记录（以便更新覆盖）
* 索引 `cosine` 用于按相似度排序或范围筛选（如获取高相似度候选）

**业务注意**

* 系统在计算相似度时：若当天已存在该配对记录则直接复用；否则会计算并存储。此策略减少同一天内的重复计算。
