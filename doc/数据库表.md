...
> 重要说明（2026-03-18）：本文件包含历史演进示例，部分字段（如 `Survey.link`、`AppUser.school/tags/token`）已不属于当前 `core/models.py` 的真实结构。
> 数据库字段与约束请始终以 `core/models.py` 和迁移文件为准。
...
```json
{
	"status": {
		...
		"db_current": "string",
		"note": "当前管理接口对外主要归一为 draft/live/paused/ended；兼容入口仍可能出现 closed 等内部值。"
	},
	"updated_at": {
...
```json
{
	...
	"differences": {
		...
		"Survey.status": {
			"api_enum": ["draft", "live", "paused", "ended"],
			"db": "varchar(32)",
			"action": "管理接口层已进行状态归一；兼容入口存在少量内部状态差异。"
		},
		"SurveyDraft": {
