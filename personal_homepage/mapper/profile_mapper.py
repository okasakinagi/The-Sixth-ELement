    ...
    def get_user_profile_dict(user):
        ...
        for user_tag in user_tags:
            ...
            if tag_type == UserProfileMapper.TAG_TYPE_GENDER and not profile["gender"]:
                ...
                tag_type == UserProfileMapper.TAG_TYPE_COLLEGE
                and not profile["college"]
            ):
                ...
                tag_type == UserProfileMapper.TAG_TYPE_STATUS
                and not profile["current_status"]
            ):
    ...
    def update_user_profile(user, profile_data):
        ...
        with transaction.atomic():
            ...
            if "gender" in profile_data:
                UserProfileMapper.set_user_tag(
                    user, UserProfileMapper.TAG_TYPE_GENDER, profile_data["gender"]
                )
            if "age" in profile_data:
                age_str = (
                    str(profile_data["age"])
                    if profile_data["age"] is not None
                    else None
                )
                UserProfileMapper.set_user_tag(
                    user, UserProfileMapper.TAG_TYPE_AGE, age_str
                )
            if "grade" in profile_data:
                UserProfileMapper.set_user_tag(
                    user, UserProfileMapper.TAG_TYPE_GRADE, profile_data["grade"]
                )
            if "college" in profile_data:
                UserProfileMapper.set_user_tag(
                    user, UserProfileMapper.TAG_TYPE_COLLEGE, profile_data["college"]
                )
            if "major" in profile_data:
                UserProfileMapper.set_user_tag(
                    user, UserProfileMapper.TAG_TYPE_MAJOR, profile_data["major"]
                )
            if "mbti" in profile_data:
                UserProfileMapper.set_user_tag(
                    user, UserProfileMapper.TAG_TYPE_MBTI, profile_data["mbti"]
                )
            if "current_status" in profile_data:
                UserProfileMapper.set_user_tag(
                    user,
                    UserProfileMapper.TAG_TYPE_STATUS,
                    profile_data["current_status"],
                )

            # 多值字段（数组）
            if "interests" in profile_data:
                UserProfileMapper.set_user_tags_multi(
                    user, UserProfileMapper.TAG_TYPE_INTEREST, profile_data["interests"]
                )
